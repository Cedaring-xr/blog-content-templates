...
**The most useful CLI commands with Amplify are:** - amplify configure - configure access credentials, attach account and user profile - amplify init - initialize a new project using amplify - amplify add storage - create a database connection to DynamoDB tables - amplify add api - - amplify build - provisions cloud resources with the latest local developments - amplify push - pushes local build settings to cloud resources
